...
;; $Id: flash-paren.el,v 1.22 2019/10/04 20:54:13 friedman Exp $
        ...
        (flash-paren-while-no-input
          (while (and (= (point) opoint)
                      (cond ((fboundp 'redisplay)
                             ;; Wrappingg this in a save-excursion seems to
                             ;; prevent Emacs from moving the point in one
                             ;; window to the point in another window that
                             ;; is displaying the same buffer.  Perhaps
                             ;; while-no-input is interrupting redisplay in
                             ;; a weird state?
                             (save-excursion (redisplay))
